...
eucalyptus <- read("./data/eucalyptus.csv")
skimr::skim(eucalyptus)
```

```{r desccomment, output='asis'}
select_answer(r"-{
[x] -   Ce jeu de données ne contient aucune valeur manquante.
[] -   Une valeur est manquante dans ce jeu de données.
[] -   Plusieurs valeurs sont manquantes dans ce jeu de données.

[x] -   Ce tableau inclut uniquement des variables numériques.
...
eucalyptus <- labelise(eucalyptus,
  label = list(ht = "Hauteur", circ = "Circonférence à 1,5m"),
  units = list(ht = "m", circ = "cm")
)
```
...
[x] -   Le nuage de points s'étire en oblique de façon à peu près linéaire.
...
[x] -   Des sous-groupes sont visibles sur le graphique.
  
[] -   Aucune valeur extrême ne se détache du nuage de points.
[] -   Une valeur extrême se détache du nuage de points.
[x] -   Plusieurs valeurs extrêmes se détachent du nuage de points.}-")
```
...
```{r corrp, record='RN'}
cor(eucalyptus$ht, eucalyptus$circ)
```
...
```{r corrs, record='RN'}
cor(eucalyptus$ht, eucalyptus$circ, method = "spearman")
```

```{r corrcomment, output='asis'}
select_answer(r"-{
[x] -   Les deux coefficients de corrélation sont proches l'un de l'autre, signe que le nuage de points est pratiquement linéaire.
...
[x] -   Les coefficients de corrélation sont élevés.
...
cor.test(data = eucalyptus,~ ht + circ, alternative = "two.sided" )
```
...
[x] -   La corrélation entre la hauteur et la circonférence est significative au seuil $\alpha$ de 5%. Étant donné la valeur du coefficient de corrélation, nous devons nous méfier de la multicolinéarité dans le modèle.}-")
```
